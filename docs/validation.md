...
| ARM64 与 Intel 自动化路径 | GitHub-hosted `macos-15` / Python 3.13 与 `macos-15-intel` / Python 3.9 | v0.2.0 的 44 项测试、字段长度、可复现构建、校验和与两个 ZipApp 入口冒烟通过 |
| `codex-model` 快捷启动 | 临时配置 + 假 Codex | GPT/DeepSeek 路由、项目路径、参数透传、首次配置失败关闭通过；真实外部网络仍未调用 |
| 一键安装 | 匿名公开下载 + 临时安装目录 | v0.2.0 Release 两项资产 SHA-256 通过且与本地构建逐字节一致；两个入口、GPT dry-run 与非 HTTPS 拒绝通过 |
...
因此，当前可以称为“Apple silicon 本机只读基线、ARM64/Intel macOS 15 自动化路径、公开 v0.2.0 安装与 GPT dry-run 已验证”。不能称为“所有 Mac 通用”“所有 Codex 版本兼容”“Keychain 真实写入闭环已验证”或“DeepSeek 端到端已验证”。
